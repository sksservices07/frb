...
const UNISWAP_ROUTER = process.env.UNISWAP_ROUTER //contract of pancake router
const ETH_CONTRACT = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2' //Contract on WETH coin
const environment = 'prod'
const wss =
  environment === 'test'
    ? process.env.WSS_ETHER_TEST
    : process.env.WSS_ETHER_MAIN
console.log('[WSS]: ', wss)
...
function router(signerOrProvider) {
  return new ethers.Contract(UNISWAP_ROUTER, router_abi, signerOrProvider)
}
...
async function get_token_creation_time(token_address) {
  ...
  const res = await axios.get(
    `https://api.etherscan.com/api?module=account&action=txlist&address=${token_address}&startblock=0&endblock=99999999&page=1&offset=10&sort=asc&apikey=5MED8XQ8UWUSPJCHRCPU543ZVWS5YESX7E`
  )
...
async function buyToken(provider, signer, token_address, gasLimit, gasPrices) {
  ...
  let result = true
  try {
    // const nonce = await provider.getTransactionCount(UNISWAP_ROUTER)
...
async function sellToken(
  ...
  if (
    ...
    console.log(
      `[SELL APPROVED]: https://etherscan.com/tx/${receipt_approve.transactionHash}`
    )
...
const init = async function () {
  ...
  const customRpcProvider = new ethers.providers.JsonRpcProvider(
    'https://mainnet.infura.io/v3/ecf0a9d17781402a8bc3aeec89e8f667'
  )
        ...
        if (value > 0.00001) {
          ...
          if (result.length > 0) {
            if (result[1].length > 0) {
              ...
              const buyGasPriceInEth = parseFloat(
                ethers.utils.formatEther(buyGasPrice)
              )
              const sellGasPriceInEth = parseFloat(
                ethers.utils.formatEther(sellGasPrice)
              )

              const change_eth =
                parseFloat(sell_output) -
                BUY_AMOUNT -
                (buyGasPriceInEth + sellGasPriceInEth)
